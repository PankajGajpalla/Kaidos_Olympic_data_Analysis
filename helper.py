...
def most_successful(df, sport):
    temp_df = df[(df["Medal"] == "Gold") | (df['Medal']=="Silver") | (df['Medal'] == "Bronze")]

    if sport != "Overall":
        temp_df = temp_df[temp_df['Sport'] == sport]

    x = temp_df['Name'].value_counts().reset_index().head(10).merge(df, on='Name', how='left').drop_duplicates("Name")
    x.rename(columns={"count":"Medals"}, inplace=True)
    return x[['Name', 'Sport', 'region', 'Medals']]
